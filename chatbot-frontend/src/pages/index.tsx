...
export default function Home() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (input.toLowerCase().startsWith("delete file ")) {
        ...
        // Use Electron IPC instead of HTTP request
        const result = await window.electron.invoke('delete-file', { filename, targetDir });
      ...
      // Check for file finding
        ...
        const fetchResponse = await fetch("http://127.0.0.1:8000/find-file", {
          method: "POST",
      ...
      // Check for music requests
        ...
        const fetchResponse = await fetch(`http://127.0.0.1:8000/spotify/${endpoint}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      // Default to chat
        ...
        try {
          const fetchResponse = await fetch("http://127.0.0.1:8000/chat", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
      ...
      if ('error' in response) {
        botMessage = { role: "bot", content: response.error };
      } else if ('success' in response) {
        botMessage = { role: "bot", content: response.success };
      } else if ('response' in response) {
        botMessage = { role: "bot", content: response.response };
      } else {
        botMessage = { role: "bot", content: "Sorry, I couldn't process that request." };
      }
  ...
  const submitFileContent = async () => {
    ...
    try {
        // Use Electron IPC instead of HTTP request
        const result = await window.electron.invoke('create-file', {
            filename,
            content: fileContent,
            targetDir
        });

        const botMessage = { role: "bot", content: result.message };
        setMessages((prev) => [...prev, botMessage]);
    } catch (error) {
        console.error("Error:", error);
        const errorMessage = { role: "bot", content: "Sorry, I encountered an error while creating the file." };
        setMessages((prev) => [...prev, errorMessage]);
    }
